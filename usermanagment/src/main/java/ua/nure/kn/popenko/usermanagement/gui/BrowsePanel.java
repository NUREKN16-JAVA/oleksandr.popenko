...
public class BrowsePanel extends JPanel implements ActionListener {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 1L; // dunno if it works
	...
	public JTable getUserTable() {
		if (userTable == null) {
			...
			userTable.setName(Messages.getString("userTable")); //$NON-NLS-1$
		}
		return userTable;
	}
	...
	public void initTable() {
		...
		try {
			...
			model = new UserTableModel(new ArrayList<Object>());
	...
	public void actionPerformed(ActionEvent e) {
		...
		if ("add".equalsIgnoreCase(actionCommand)) { //$NON-NLS-1$
			...
			if (selectedRow == -1) {
				JOptionPane.showMessageDialog(this, Messages.getString("BrowsePanel.ñhoose_user_edit")
						, "Edit", JOptionPane.INFORMATION_MESSAGE); //$NON-NLS-1$ //$NON-NLS-2$
				return;
			}
			...
			int selectedRow = userTable.getSelectedRow();
			if (selectedRow == -1) {
				JOptionPane.showMessageDialog(this, Messages.getString("BrowsePanel.ñhoose_user_delete")
						, "Delete", JOptionPane.INFORMATION_MESSAGE); //$NON-NLS-1$ //$NON-NLS-2$
				return;
			}
